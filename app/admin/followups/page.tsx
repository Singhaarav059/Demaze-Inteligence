...
// ============================================================
// Promoted to primary nav in the 2026-08-31 UX restructuring (moved from
// /admin/outbound/followups, which now redirects here) - this is the one
// recurring cross-company job ("who's due for a follow-up today") that
// doesn't fit inside working a single company in Work. Per-contact view of
// everyone still owed a follow-up: what's due and when, manual "Send Now"
// (bypasses the schedule) / "Stop Remaining" (cancels the rest of the
// sequence for that one contact) actions, inline editing of the next
// follow-up's copy before it goes out, and a global cadence editor.
//
// Send Now can trigger a REAL send once a real sending provider (e.g.
// Gmail) is active - same standing rule as the rest of outbound send
// (CLAUDE.md's per-batch confirmation requirement), so it's gated behind
// the same ConfirmDialog + real-provider-aware copy. Stop Remaining isn't a
// send, but is a one-way door (no "resume" action), so it gets its own
// confirm too.
// ============================================================
...
import { StatusDot } from '@/app/admin/outbound/StatusDot'
    ...
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-8">
      <div>
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          Who&apos;s due for a follow-up, and what to do about it.
        </p>
      </div>
      ...
      <GuideNote>
        <p>
          Everyone still owed a follow-up, across all your outreach, sorted by what&apos;s due
          soonest. <strong>Send Now</strong> bypasses the schedule; <strong>Stop Remaining</strong>{' '}
          cancels the rest of that person&apos;s sequence and can&apos;t be undone from this page.
        </p>
      </GuideNote>
      ...
      >
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground/70">
              Days since the previous send before each step is due. Applies to everything you send.
            </p>
          ...
          <div className="space-y-3 pt-3 border-t border-border">
            ...
            {engineTickSummary && (
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-muted-foreground pt-1 sm:grid-cols-3">
                <span>Checked: <span className="text-foreground">{engineTickSummary.campaignsChecked}</span></span>
      ...
      {loading ? (
        ...
        <EmptyState icon={Mail} title="No one is currently owed a follow-up" />
      ) : (
                        ...
                        <CardContent className="px-5 py-4 space-y-3">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-xs text-muted-foreground/60 mt-0.5 truncate">
                                {row.campaignName}
                                {row.campaignPaused && <Badge variant="secondary" className="ml-1.5">paused</Badge>}
                              </div>
                            </div>
                          ...
                          {row.draftBody === null ? (
                            <p className="text-xs text-muted-foreground/60 italic">
                              No follow-up message drafted yet for this step.
                            </p>
                          ) : (
