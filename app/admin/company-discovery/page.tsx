...
import { Building2, MapPin, Filter, HelpCircle, Search, Clock, ChevronRight } from 'lucide-react'
...
function Checkbox({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return (
    <label className="flex items-center gap-1.5 text-xs text-muted-foreground/90 cursor-pointer">
    ...
    <div className="mx-auto max-w-5xl px-6 py-8 space-y-6">
      <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="space-y-3">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Discover companies worth researching</h1>
            <p className="text-sm text-muted-foreground mt-1 max-w-lg">
              Define your target market and let Demaze find companies that match.
            </p>
          </div>
      ...
      <div ref={searchCardRef}>
        <Card className="bg-card border-border">
          <CardContent className="px-5 py-5 space-y-5">
            <div>
              <h2 className="text-sm font-semibold text-foreground">Find Companies</h2>
              <p className="text-muted-foreground/70 text-xs mt-0.5">Define the companies Demaze should target. We&rsquo;ll find companies that match your criteria.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="employee-range-select">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="revenue-range-select">
            ...
            <div className="space-y-1.5">
              <Label className="gap-1.5">
                <MapPin className="size-3.5 text-muted-foreground" /> Headquarters <span className="text-muted-foreground/60 font-normal">(optional)</span>
              </Label>
              <p className="text-muted-foreground/50 text-[11px]">Company headquarters, not customer or sales regions.</p>
            ...
            <div className="flex items-center gap-3 flex-wrap pt-1">
              <Button onClick={runSearch} disabled={searching || !sector}>
                {searching ? <><Spinner /> Finding matching companies…</> : <><Search className="size-3.5" /> Find Companies</>}
              </Button>
