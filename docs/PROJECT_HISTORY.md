...
**RESOLVED 2026-08-10 (stale note corrected 2026-08-24)** — this was fixed,
just never updated here. `detectPageType()` now anchors every keyword group
(including `'about'`) with a `SEG_END = '(?=[/.?]|$)'` lookahead, so
`/company` only matches when followed by `/`, `.`, `?`, or end-of-string —
`/blog/company-news` now correctly falls through to `'blog'` instead of
`'about'`. Deliberately excludes `-` as a boundary char to avoid
reintroducing the same collision via slugs like `company-news`. Covered by
`tests/evidence-extractor-pagetype.test.ts`'s `describe('detectPageType —
segment-boundary collisions')` block.
...
2026-07-24 audit's ranked list. `detectPageType()`'s missing segment-boundary
anchoring (see above) was RESOLVED 2026-08-10 — stale "not fixed" reference
here corrected 2026-08-24. The non-English/diacritic-
...
**RESOLVED 2026-08-24 — 5 of the 6 flagged pages got the deferred
card-based mobile treatment** (`auto-gtm/ContactInfoRow` was not part of
this session's task list and stays open). Read each file's actual current
structure first, per the task's own instruction, rather than assuming a
generic rewrite was needed — most of these were already `Card`/flex-row
based, not literal `<table>`s, so the real fix was almost always a
targeted Tailwind responsive-class change on existing markup, not a new
component:
- `run-history/page.tsx` — the Research-tab row's title block and its
  action-button cluster (View Report / Resume in Auto Flow / delete /
  expand) now stack (`flex-col` → `sm:flex-row`) instead of competing for
  one fixed-height row; the action buttons also gained `flex-wrap` and lost
  their opacity-on-hover-only gating below `sm` (no hover on touch, so they
  need to just be visible).
- `outbound/overview/page.tsx` — the 5-column Emails `<table>` (previously
  wrapped in `overflow-x-auto` only, meaning "scroll sideways," not a real
  card layout) is now `hidden md:block`; a new `md:hidden` stacked card
  list renders the same row data (contact, company/email, subject,
  campaign link, `StatusDot`, date) using the exact same `emails` array and
  `statusTone`/`statusLabel`/`formatDate` helpers already in the file.
- `outbound/followups/page.tsx` — the per-contact `Card`'s info block
  (name/email/campaign) gained `min-w-0 flex-1` + `truncate` (previously
  had neither, a real overflow risk for a long name/email/campaign name
  since flex items default to `min-width: auto`); the Send Now / Stop
  Remaining Follow-ups button row gained `flex-wrap`.
- `company-discovery/CompanyMatchList.tsx` — the per-company row (checkbox
  + avatar + name/domain on one side, status + signal count + action links
  on the other) now stacks below `sm` (`flex-col` → `sm:flex-row`); the
  status/actions block switches from a vertical `flex-col items-end` to a
  wrapped horizontal `flex-row flex-wrap justify-end` on mobile (with a
  `pl-8` nudge to roughly align under the avatar) so it reads as a
  right-aligned action row under the title instead of an empty vertical
  column fighting the title for width.
- `intelligence-lab/ComparisonPanel.tsx` — the 3-column Metric/Slot A/Slot
  B `<table>` is now `hidden sm:block`; a new `sm:hidden` stacked list
  renders each metric as its own bordered block (label, then "A: ... / B:
  ..." on one line) using the same `rows`/`fn(a)`/`fn(b)` data the table
  already computes.
- `intelligence-lab/page.tsx` — investigated, changed nothing. Grepped the
  whole file for `grid-cols-\d+` and `<table`: every dense grid in this
  file (Company Overview, Scores, Business Model Analysis, Signal
  Clusters, Executive Brief, Outreach Intelligence, etc.) already uses
  `grid-cols-1 sm:grid-cols-N`/`lg:grid-cols-N` responsive classes that
  correctly collapse to a single column below `sm`/`lg`, and there is no
  literal `<table>` anywhere in the file (only `<pre>` blocks for raw
  JSON/markdown, already inside `overflow-y-auto`/`overflow-x-auto`
  containers). This page was already mobile-safe; per the ladder's first
  rung ("does this need to exist at all"), no change was made.

**Verification**: `tsc --noEmit` clean, full suite 915/915 (no new tests —
pure Tailwind responsive-class changes to already-tested markup, no new
business logic, consistent with this file's own precedent for pure-UI
changes). Live-verified with a headless Playwright pass (no MCP/computer-
tool browser was available in this session, so a throwaway local
Playwright script was used instead, pointed at an already-running
`next dev` on port 3000 rather than starting a second one) at 375×812 and
1280×800 across all 6 pages: `document.documentElement.scrollWidth ===
clientWidth` (zero horizontal overflow) on every one of the 12
page/viewport combinations, zero console/page errors. Real, non-empty data
was confirmed rendering correctly in the new mobile card layouts, not just
the empty state: `run-history` (100 real saved runs, action buttons
correctly wrapping onto their own row under the title), `outbound/overview`
(20 real queued/sent/stopped emails rendering as cards with campaign links
intact), `outbound/followups` (a real overdue Larsen & Toubro follow-up,
expanded to show its editable draft body and stacked Send Now/Stop
buttons). Desktop screenshots at 1280×800 confirmed the original
table/row layouts are unchanged (`hidden md:block`/`hidden sm:block`
correctly gate the new mobile markup away from desktop).

**Not fully verified — one honest gap**: `company-discovery/
CompanyMatchList.tsx`'s mobile row layout was confirmed overflow-free in
its pre-search and mid-search-loading states, and the JSX change itself
was verified by direct reading (checkbox+avatar+name stays inline, the
status/actions block correctly moves below on mobile), but a populated
list of real matched companies was never captured on screen — clicking a
"Recent Search" on this page re-runs a live search rather than replaying a
cached result (consistent with this file's own note elsewhere that batch
search state is in-memory React state, never persisted), and this session
did not spend real discovery-API quota to force that state without
explicit confirmation first, per this file's own quota-spending
discipline. `outbound/followups`'s desktop view and `company-discovery`'s
desktop view were spot-checked for no regression but not exhaustively
re-verified against every interactive state.
